...
5. 点击记录查看请求头、响应头、正文和错误；可将单条记录导出为 JSON。长按记录可选择“复制为 cURL”，成功后显示提示。生成命令用于 POSIX shell（如 macOS/Linux），包含 URL、方法、请求头和已保存的正文；二进制正文通过 base64 管道传入。Content-Length 和逐跳请求头由 cURL 重新处理；请求被截断时提示无法生成完整命令。
...
- path 默认区分大小写，不含 query。`/api` 精确匹配；`/api/*` 匹配前缀 `/api/`。以 `re:` 开头使用 Go 正则语法：`re:scan` 匹配路径任意位置的 scan；`re:(?i)scan` 忽略大小写；`re:^/api/(scan|qrcode)/?$` 匹配整个指定路径。多行规则取并集，可混用三种语法。正则在保存时校验并编译，错误时保留原配置；不支持前后查找和反向引用。按 URL 中的转义路径匹配，不做 URL 解码或路径归一化，正则也不匹配 query。
