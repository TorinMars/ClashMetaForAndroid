...
- 抓包应用名单独立于分应用路由名单。开启抓包时，VPN 接管范围自动包含抓包应用；空抓包名单表示接管全部应用。原分应用路由设置不变，原本不经过 VPN 的应用仅为抓包被接管，所有 TCP/UDP 转发（含未匹配的请求）强制 DIRECT；原接管范围内的应用继续使用原 Clash 规则。校验上游 TLS 证书，不跳过校验。
- 开启/停止抓包或修改抓包应用名单时，自动重建 VPN 接管范围，现有连接可能重连；此重建保留抓包启用状态。抓包期间临时关闭系统 HTTP 代理入口，停止后恢复原设置。仍使用本地 VpnService，不能与其他 VPN 同时工作；绕过私有网络、IPv6 和应用主动绕过 VPN 等设置仍影响接管。扩大接管范围时若无法识别 UID，拒绝该连接，避免将原代理应用误放直连。
...
点击“一键复制全部请求为 Shell 脚本”，复制当前保留的请求快照（最多 100 条），按请求开始时间排序。保存为 `requests.sh` 后在 macOS/Linux 等 POSIX shell 中执行 `sh requests.sh`，需有 curl 和 base64。命令会重新发起原请求，包括有写入作用的请求；不会自动执行。单条网络失败后继续下一条，最终有网络失败时退出码为 1；每条连接超时 15 秒、总超时 120 秒，可自行编辑。HTTP 错误状态码保留 curl 默认行为。

已截断或无法转换的记录会注明并跳过，复制提示显示包含/跳过数量。抓包中的新记录不会改变本次快照。脚本过大时改为文件保存，避免 Android 剪贴板事务大小限制。已淘汰或清空的请求无法恢复。
